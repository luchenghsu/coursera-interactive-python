...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            canvas.draw_image(self.image, [self.image_center[0]+self.age * self.image_size[1], self.image_center[0]], self.image_size,
                              self.pos, self.image_size, self.angle)
        else:    
            canvas.draw_image(self.image, self.image_center, self.image_size,
                              self.pos, self.image_size, self.angle)
    ...
    def collide(self, other_object):
        d = dist(self.pos, other_object.get_postion())
...
def group_collide(group, other_object):
    ...
    for sp in list(group):
        if sp.collide(other_object):
            group.remove(sp)
            a_explosion = Sprite(sp.get_postion(), sp.get_velocity(), 0, sp.get_angular_velocity(), explosion_image, explosion_info)
...
def draw(canvas):
    ...
    process_sprite_group(canvas, missle_group)
    ...
    if not started:
        ...
        if group_collide(rock_group, my_ship):
            ...
            if lives == 0:
                started = False
                group_clear(rock_group)
                group_clear(explosion_group)
                group_clear(missle_group)
...
# initialize ship and two sets of sprite
